...
[Functions]
  ...
  [BiSbTeSe_S_func]
    type = ParsedFunction
    expression = '(3.064066e-7*t*t*t*t -4.975877e-4 *t*t*t+ 0.2874121*t*t - 69.79877 * t +6253.741) *1e-6'
  []
  [BiSbTeSe_S_func_n]
    type = ParsedFunction
    expression = '(3.064066e-7*t*t*t*t -4.975877e-4 *t*t*t+ 0.2874121*t*t - 69.79877 * t +6253.741) *1e-6* -1'
  []
  [BiSbTeSe_R_func]
    type = ParsedFunction
    expression = '(4.429348e-9 *t*t*t*t -9.117995e-6*t*t*t + 0.006777 * t *t - 2.157869*t + 252.662979) *1e-5'
  []
  [BiSbTeSe_k_func]
    type = ParsedFunction
    expression = '(3.612223e-10 *t*t*t*t -5.247370e-7*t*t*t+2.636298e-4*t*t - 5.155722e-2 * t + 3.419570)'
  []
[]
...
[Materials]
  ...
  [nleg_thermal]
    ...
    temp = T
    thermal_conductivity_temperature_function = BiSbTeSe_k_func
  ...
  [pleg]
    ...
    temp = T
    seebeck_temperature_function =BiSbTeSe_S_func
    resistivity_temperature_function =BiSbTeSe_R_func
    block = 'p_leg 104 204'
    # outputs = exodus
  ...
  [nleg]
    ...
    type = ADThermalElectricMaterial
    temp = T
    seebeck_temperature_function = BiSbTeSe_S_func_n
    resistivity_temperature_function = BiSbTeSe_R_func
    block = 'n_leg 102 202'
    # outputs = exodus
    # output_properties = 'seebeck'
