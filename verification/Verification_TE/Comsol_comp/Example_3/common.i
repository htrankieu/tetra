

[Mesh]
[cmg]
    type                  = CartesianMeshGenerator
    dim                   = 2
    dx                    = '1'
    dy                    = '0.1 5.8 0.1'
    ix                    = '10'
    iy                    = '3 42 3'
    subdomain_id          = '1
                             2
                             3'
  []
  [rename_blocks]
    type                  = RenameBlockGenerator
    input                 = 'cmg'
    old_block             = '1 2 3'
    new_block             = 'interconnect_cold p_leg interconnect_hot'
  []


    [extrude]
    type = AdvancedExtruderGenerator
    input = rename_blocks
    heights = '1'
    num_layers = '10'
    direction = '0 0 1'
  []

  [EntireMesh]
    type = TransformGenerator
    input = extrude
    transform = SCALE
    vector_value = '0.001 0.001 0.001'
  []
    [align_comsol]
    type = TransformGenerator
    input = EntireMesh
    transform = ROTATE
    vector_value = '-90 90 0'
  []
  # uniform_refine = 2
[]

[Functions]

  # [pleg_seebeck]
  #   type = PiecewiseLinear
  #   data_file = 'seebeck_pleg.csv'
  #   x_index_in_file = 0
  #   y_index_in_file = 1
  #   xy_in_file_only = true
  #   format = columns
  #   # scale_factor = 1e-6
  # []
  # [nleg_seebeck]
  #   type = ParsedFunction
  #   symbol_names = 'seebeck'
  #   symbol_values = 'pleg_seebeck'
  #   expression = '-1*seebeck'
  # []
  # [leg_k]
  #   type = PiecewiseLinear
  #   data_file = 'k_pleg.csv'
  #   x_index_in_file = 0
  #   y_index_in_file = 1
  #   xy_in_file_only = true
  #   format = columns
  # []
  # [leg_sigma]
  #   type = PiecewiseLinear
  #   data_file = 'sigma_pleg.csv'
  #   x_index_in_file = 0
  #   y_index_in_file = 1
  #   xy_in_file_only = true
  #   format = columns
  #   # scale_factor = 1000
  # []
  # [leg_resistivity]
  #   type = ParsedFunction
  #   symbol_names = 'sigma'
  #   symbol_values = 'leg_sigma'
  #   expression = '1/sigma'
  # []
[]

[Variables]
  [T]
  []
  [elec]
  []
[]

[Kernels]
  # Heat DT
  [HeatDiff]
    type = ADHeatConduction
    variable = T
  []
  [electric]
    type = ElectricConduction
    variable = elec
  []
  [seebeck_effect]
    type = SeebeckEnergy
    variable = elec
    temp =T
  []
  [Peltier_effect]
    type = PeltierHeat
    variable = T
  []
  [HeatSrc]
    type = TEJouleHeat
    variable = T
    current_density = current_density
    elec = elec
  []
[]

[BCs]
  [cold_temp]
    type = ADDirichletBC
    variable = T
    boundary = 'bottom'
    value = ${fparse 0 + 273.15}
    # heat_transfer_coefficient = 1e12
  []
  [hot_temp]
        type = ADDirichletBC
        value = ${fparse 100 + 273.15}

    variable = T
    boundary = 'top'
    # T_infinity = ${fparse 100 + 273.15}
    # heat_transfer_coefficient = 1e12
  []
  [elec_right]
    type = ADDirichletBC
    variable = elec
    boundary = bottom
    value = 0
  []
  [intensity]
    type = ADFunctionNeumannBC
    function = current_density_fn

    variable = elec
    boundary = top
    # value = ${fparse -I/(0.001*0.001)}
  []
[]




[Materials]
  [J_mat]
    type = CurrentDensityMaterial
    temp = T
    elec = elec
  []
  [pleg_thermal]
    type = ADHeatConductionMaterial
    temperature = T
    thermal_conductivity_temperature_function = leg_k
    specific_heat = 154.4
    block = 'p_leg'
  []
    [pleg_density]
    type = ADGenericConstantMaterial
    prop_names = 'density'
    prop_values = 7740
    block = p_leg
  []
 [pleg]
    type = ADThermalElectricMaterial
    temp = T
    seebeck_temperature_function = pleg_seebeck
    resistivity_temperature_function =leg_resistivity
    block = p_leg
  []
  [interconnect_th]
    # Copper
    type = ADHeatConductionMaterial
    temperature = T
    thermal_conductivity = 350 # W/mK
    specific_heat = 385 # J/kgK
    block =  'interconnect_cold interconnect_hot'
  []
  [interconnect_density]
    type = ADGenericConstantMaterial
    prop_names = 'density'
    prop_values = 8920
    block =  'interconnect_cold interconnect_hot'
  []
  [interconnect_TE]
    type = ADThermalElectricMaterial
    temp = T
    seebeck_temperature_function = 6.5e-6
    resistivity_temperature_function = ${fparse 1/5.9e8}
    block =  'interconnect_cold interconnect_hot'
  []
[]

[Postprocessors]
  [Vbottom]
    type = SideAverageValue
    boundary = bottom
    variable = elec
  []
  [Vtop]
    type = SideAverageValue
    boundary = top
    variable = elec
  []
  [U_load]
    type = ParsedPostprocessor
    pp_names = 'Vtop Vbottom'
    expression = ' Vbottom - Vtop '
  []

  [heat_in]
    type = ADSideDiffusiveFluxIntegral
    variable = T
    diffusivity = thermal_conductivity
    boundary = 'top'
  []

  [I_out]
    type = ADSideIntegralMaterialProperty
    property = current_density
    component = 0
    boundary = 'bottom'
  []
  [P_load]
    type = ParsedPostprocessor
    pp_names = 'I_out U_load'
    expression = 'U_load*I_out'
  []

  [T_bottom]
    type = SideAverageValue
    boundary = bottom
    variable = T
  []
  [T_top]
    type = SideAverageValue
    boundary = top
    variable = T
  []
  [delta_T]
    type = ParsedPostprocessor
    pp_names = 'T_top T_bottom'
    expression = 'T_top - T_bottom'
  []
[]

[Preconditioning]
  [SMP]
    type = SMP
    full = true
  []
[]

[Outputs]
  [out]
    type = Exodus
    output_material_properties = true
  []
  [csv]
    type = CSV
    create_final_symlink = true
    # execute_on = 'FINAL'
  []

[]

# [VectorPostprocessors]
#   [line]
#     type = LineValueSampler
#     sort_by = 'x'
#     start_point = '0.0001 -0.0005 -0.0005'
#     end_point = '0.0059 -0.0005 -0.0005'
#     num_points = 120
#     variable = 'T elec'
#     execute_on = FINAL
#   []
# []
